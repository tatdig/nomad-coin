...
#include "nomadaddressvalidator.h"
...
{
    widget->setMaxLength(NomadAddressValidator::MaxAddressLength);
    widget->setValidator(new NomadAddressValidator(parent));
    widget->setFont(nomadAddressFont());
}
...
bool parseNomadURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("nomad"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!NomadUnits::parse(NomadUnits::NMD, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseNomadURI(QString uri, SendCoinsRecipient *out)
{
    // Convert nomad:// to nomad:
    //
    //    Cannot handle this later, because nomad:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("nomad://"))
    {
        uri.replace(0, 12, "nomad:");
    }
    QUrl uriInstance(uri);
    return parseNomadURI(uriInstance, out);
}
...
void openConfigfile()
{
    ...
    /* Open nomad.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Nomad.lnk";
}
...
{
    return GetAutostartDir() / "nomad.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a nomad.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Nomad\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Nomad-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  nomad-qt [" + tr("command-line options") + "]                     " + "\n";
