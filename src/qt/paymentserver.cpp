...
const int NOMAD_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString NOMAD_IPC_PREFIX("nomad:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(NOMAD_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(NOMAD_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(NOMAD_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click nomad: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start nomad: click-to-pay handler");
    else
...
{
    // clicking on nomad: URLs creates FileOpen events on the Mac:
