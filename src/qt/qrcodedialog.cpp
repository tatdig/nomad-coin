...
{
    QString ret = QString("nomad:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non NMD unit input in lnReqAmount, we generate the URI with NMD as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(NomadUnits::format(NomadUnits::NMD, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
