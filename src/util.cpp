...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Nomad
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Nomad
    // Mac: ~/Library/Application Support/Nomad
    // Unix: ~/.nomad
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nomad";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "nomad.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "nomad.conf";
               ...
               fprintf(ConfFile, "port=15716\n");
               fprintf(ConfFile, "rpcport=15717\n");
    ...
    {
        // Don't overwrite existing settings so command line settings override nomad.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "nomadd.pid"));
...
// Format the subversion field according to BIP 14 spec (https://en.nomad.it/wiki/BIP_0014)
